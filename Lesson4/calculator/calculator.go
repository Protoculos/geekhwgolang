...
//Help обрабатывает справку помощи
func Help(expr string) (string, error) {
	hp, err := parser.ParseExpr(expr)
	if err != nil {
		return "", err
	} else {
		return evalHelp(hp)
...
// Разбор полученных данных
func evalHelp(expr ast.Expr) (string, error) {
	return help(expr.(*ast.Ident))
}
...
func help(id *ast.Ident) (string, error) {
	switch n := strings.ToLower(id.Name); n {
	case "help":
		return "Для получения помощи наберите: функции, методы", nil
	case "функции":
		return "Список функций", nil
	case "методы":
		return "Список методов", nil
	default:
		return "", errors.New("Неизвестная константа " + n)
	}
}
